...
int main(int argc, char* argv[]) {
	...
	if (strcmp(argv[1], ACTION_GENERATE_KEY) == 0) { // 生成保存密钥的文件
		if (argc != 3) {
	...
	else if ((strcmp(argv[1], ACTION_ENCRYPT) == 0) || (strcmp(argv[1], ACTION_DECRYPT) == 0)) { //加解密
		if (argc != 5) {
		...
		key_set* key_sets = (key_set*)malloc(17*sizeof(key_set));//保存16轮的子密钥
